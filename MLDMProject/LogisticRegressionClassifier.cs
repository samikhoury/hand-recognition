...
namespace MLDMProject
{
    ...
    public class LogisticRegressionClassifier : IClassifier
    {
        ...
        public LogisticRegressionClassifier(int input, int output)
        {
            this.input = input;
            this.output = output;
        ...
        public void Train(List<Observation> observations)
        {
            ...
            this.model = new MultinomialLogisticRegression(this.input, this.output); 
            LowerBoundNewtonRaphson lbnr = new LowerBoundNewtonRaphson(this.model);
            int iteration = 0;
            do
            {
                error = lbnr.Run(dataInputs, dataOutputs);
                iteration++;

            } while (iteration < 250 && error > 1e-6);
        }

        public string Classify(Observation newObsrv)
        {
            string label = "";
            double[] outputs = this.model.Compute(newObsrv.efd);
